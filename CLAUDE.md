...
- **Runtime-to-empty** (branch `feature/runtime-to-empty`, unreleased): opt-in `CONFIG_BATTERY_RUNTIME_TO_EMPTY` (default n; depends on `BATTERY_SOC_COULOMB`) estimates minutes-to-empty from EMA-smoothed discharge current; exposed via native `battery_runtime_to_empty_min()`, the Zephyr `fuel_gauge` `RUNTIME_TO_EMPTY` property, and **wire v5** (38 bytes, `runtime_to_empty_min` uint32 LE @34; `UINT32_MAX`/null when idle/charging). Gateway decodes to InfluxDB field `runtime_to_empty_min`; Grafana "Time to Empty (min)" panel. Built + host/gateway tested; hardware e2e pending.
