...
- **Version**: **v0.16.0** RELEASED (swap-aware SoH Phase 1 — auto-reset learned health on a power-off battery swap via boot SoC-jump vs NVS baseline → `battery_soh_reset()` + `BATTERY_TELEMETRY_FLAG_BATTERY_SWAPPED`; opt-in `CONFIG_BATTERY_SWAP_DETECT`, default-on CR2032 only, no wire change — PR #32, hardware-validated E2E incl. false-positive control. Also bundles the Grafana "Connected Device" tile + `$device` selector, PRs #30/#31). Builds on v0.15.0 runtime-to-empty + idle-current floor (wire v5) + v0.14.0 standard Zephyr `fuel_gauge` driver + custom SoH property + v0.13.0 external-ADC voltage sense + v0.12.0 SoH NVS persistence + v0.11.0 Phase 8d State of Health + Phase 8a/8b/8c.
...
- **Read-only Zephyr fuel_gauge driver** (shipped in **v0.14.0**): opt-in `CONFIG_BATTERY_FUEL_GAUGE_API` (default n; `select FUEL_GAUGE`) exposes iBattery via the standard Zephyr `fuel_gauge` API (read-only `.get_property` only), instantiated from a DT node `compatible = "aliaksandr,ibattery-fuel-gauge"`. Custom SoH property `BATTERY_FUEL_GAUGE_PROP_SOH` carries SoH in `val->flags` (centi-percent). Builds + module-path CI-smoke-tested **and read-back validated on NUCLEO-L476RG 2026-06-04** — all properties correct vs native telemetry (`docs/captures/2026-06-04-fuel-gauge-runtime-validation.log`); opt-in serial self-check via `CONFIG_BATTERY_FUEL_GAUGE_SELFCHECK`.
- **Runtime-to-empty** (shipped in **v0.15.0**): opt-in `CONFIG_BATTERY_RUNTIME_TO_EMPTY` (default n; depends on `BATTERY_SOC_COULOMB`) estimates minutes-to-empty from EMA-smoothed discharge current; exposed via native `battery_runtime_to_empty_min()`, the Zephyr `fuel_gauge` `RUNTIME_TO_EMPTY` property, and **wire v5** (38 bytes, `runtime_to_empty_min` uint32 LE @34; `UINT32_MAX`/null when idle/charging). Gateway decodes to InfluxDB field `runtime_to_empty_min`; Grafana "Time to Empty (min)" panel. An idle-current floor (`CONFIG_BATTERY_RUNTIME_IDLE_THRESHOLD_MA_X100`, default 50 = 0.5 mA) bounds the estimate so it never spikes to multi-year values while the EMA settles after a load is removed. **Hardware-validated E2E on NUCLEO-L476RG** (firmware → wire v5 → BLE → gateway → InfluxDB → Grafana; converged to ~407 min under ~30 mA load, idle → `n/a`/field omitted — `docs/captures/2026-06-06-runtime-to-empty-idle-floor.log`).
- **Swap-aware SoH** (shipped in **v0.16.0**, Phase 1): opt-in `CONFIG_BATTERY_SWAP_DETECT` (depends on `BATTERY_SOC_SOH`; default y for CR2032 only). On boot, if SoC jumped up vs a value persisted to NVS before power-off by ≥ `CONFIG_BATTERY_SWAP_SOC_THRESHOLD_PCT_X100` (default 25%), it concludes a power-off swap → `battery_soh_reset()` + sets `BATTERY_TELEMETRY_FLAG_BATTERY_SWAPPED` (status_flags bit 7; no wire bump). New `battery_swap` module; baseline persisted throttled (≥5% drops). **Hardware-validated E2E on NUCLEO-L476RG** (PPK2 rig: seed depleted → no swap; fresh → flag + reset; same-cell power-cycle → no false positive — `docs/captures/2026-06-06-swap-aware-soh-e2e.log`). **LiPo charge-vs-swap disambiguation = Phase 2.**
- **Next milestone**: open. Candidates: **swap-aware SoH Phase 2** (LiPo charge-vs-swap disambiguation), partial-excursion learning (faster SoH convergence), external-ADC reading calibration/trim (the divider reads ~6–10% low), the deferred full hardware-validation matrix (all boards × real batteries — see `docs/ROADMAP.md`), nRF I2C remap experiment.
...
- **v5** (38 bytes): v4 + runtime_to_empty_min (uint32 LE at offset 34; `UINT32_MAX` = not available). Emitted only when `CONFIG_BATTERY_RUNTIME_TO_EMPTY=y` (`BATTERY_TELEMETRY_VERSION` is 5 then).

All five formats accepted by gateway decoder (auto-detect by length).
