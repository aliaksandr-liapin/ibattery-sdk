...
- **Version**: **v0.15.0** RELEASED (runtime-to-empty / minutes-to-empty estimate via native API + Zephyr `fuel_gauge` `RUNTIME_TO_EMPTY` property + wire v5; idle-current floor so the estimate never spikes on load removal — PRs #25, #27, both hardware-validated E2E). Builds on v0.14.0 standard Zephyr `fuel_gauge` driver + custom SoH property + v0.13.0 external-ADC voltage sense + v0.12.0 SoH NVS persistence + v0.11.0 Phase 8d State of Health + Phase 8a/8b/8c.
...
- **Read-only Zephyr fuel_gauge driver** (merged to main via PR #19, unreleased): opt-in `CONFIG_BATTERY_FUEL_GAUGE_API` (default n; `select FUEL_GAUGE`) exposes iBattery via the standard Zephyr `fuel_gauge` API (read-only `.get_property` only), instantiated from a DT node `compatible = "aliaksandr,ibattery-fuel-gauge"`. Custom SoH property `BATTERY_FUEL_GAUGE_PROP_SOH` carries SoH in `val->flags` (centi-percent). Builds + module-path CI-smoke-tested **and read-back validated on NUCLEO-L476RG 2026-06-04** — all properties correct vs native telemetry (`docs/captures/2026-06-04-fuel-gauge-runtime-validation.log`); opt-in serial self-check via `CONFIG_BATTERY_FUEL_GAUGE_SELFCHECK`.
...
125 tests (includes v4/v5 packet decoding, SoH, runtime-to-empty, scanner service-UUID matching + device-tag resolution, AnomalyDetector + CLI coverage).
