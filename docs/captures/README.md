...
| `2026-05-29-v0.10.1-ble-on-nucleo-e2e.log` | First-ever v3 telemetry over BLE on real hardware (NUCLEO + X-NUCLEO-IDB05A2 shield + INA219). Serial shows `I=2.80 mA Q=220.00→219.99 mAh`; gateway connects by **service UUID** with `name=None` and **reconnects after disconnect**; InfluxDB shows `coulomb_mah` declining `219.91→219.87` over BLE. Validates the MTU fix (27→35), gateway service-UUID matching, and the re-advertise-after-disconnect fix end-to-end. | v0.10.1 |
| `2026-05-29-v4-soh-cloud-e2e.log` | First time State of Health traversed the full pipeline (firmware→BLE→gateway→InfluxDB→Grafana) via **wire v4** (34-byte). Serial shows `[v4 ...]` packets; gateway decodes v4 and writes `soh_pct=100` to InfluxDB; the live Grafana dashboard gains a "State of Health (%)" panel. Caught a real bug the host tests missed: `BATTERY_TRANSPORT_WIRE_SIZE` was still 32, so `ble_send()` rejected 34-byte v4 with `INVALID_ARG` (`Transport send failed: -2`) — fixed + guarded with a `_Static_assert`. | (unreleased) |
