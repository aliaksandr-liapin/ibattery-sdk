...
**Firmware builds** require the nRF Connect SDK. See the [README](index.md) for details.
...
python -m pytest tests/ -v    # Run gateway tests (122 tests)
```
...
The SDK follows a strict layered architecture. See [ARCHITECTURE.md](ARCHITECTURE.md) for full details.
...
See the step-by-step guide in [BATTERY_PROFILES.md](BATTERY_PROFILES.md#adding-a-new-battery-profile).
...
5. See existing ports as reference: STM32L476 (VREFINT path) and ESP32-C3 (voltage divider path) in `battery_adc_platform.h`, board overlays and configs in `app/boards/`
6. See the portability section of [ARCHITECTURE.md](ARCHITECTURE.md) for details
...
# Gateway tests (122 Python tests)
