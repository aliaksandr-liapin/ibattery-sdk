...
## Current Status: Phase 8 + State of Health (persistent, hardware-excursion-validated), end-to-end (v0.13.0)
...
| BLE E2E | v3 telemetry over BLE: firmware → gateway → InfluxDB → Grafana | Done (v0.10.1) — validated on NUCLEO-L476RG + X-NUCLEO-IDB05A2 shield; fixed BLE MTU (27→35), gateway service-UUID matching, and re-advertise-after-disconnect |
...
- Host-based unit tests (Unity framework, 25 C test suites + 122 Python tests)
...
```ini
; platformio.ini
lib_deps = aliaksandr-liapin/ibattery-sdk@^0.13.0
```

v0.13.0 adds opt-in external-ADC battery-voltage sensing, on top of v0.12.0's SoH NVS persistence, v0.11.0's Phase 8d SoH (on-device learning + wire v4 + Grafana panel), and Phases 8a–8c.
